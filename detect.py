...
ambulance = () #엠뷸런스 좌표 초기화

# Custom Function
def draw_grid(img, grid_size=10, color=(0, 0, 255), thickness=1):
    """
    img: 입력 이미지
    grid_size: 격자의 크기 (픽셀 단위)
    color: 격자의 색상 (BGR 형식의 튜플)
    thickness: 격자의 두께 (픽셀 단위)
    """
...
def draw_nodes_with_numbers(img, nodes_list, nearest_node_index, coordinate_root_list, radius=30, thickness=5, font_scale=1.5):
    ...
    for i, nodes in enumerate(nodes_list):
        for j, node in enumerate(nodes):
            color = (40, 230, 50)  # 기본 색상
            if j == nearest_node_index:  # nearest_node_index와 동일한 인덱스의 노드인 경우
                color = (0, 0, 255)  # 변경된 색상
...
def find_nearest_node(nodes_list, ambulance):
    ...
    for index, node in enumerate(nodes_list[0]):
        distance = math.sqrt((node[0] - ambulance[0]) ** 2 + (node[1] - ambulance[1]) ** 2) #유클리드 거리 공식
...
def remote_procedure_call():
    if check_predict_ambulance:
        ...
        if nearest_node is not None:  # nearest_node가 None이 아닌지 확인
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            ambulance_node_number = None  # None으로 초기화
            coordinate_root_list = []
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if names[c] == 'ambulance':
                            ...
                            node_coordinate, ambulance_node_number = find_nearest_node(nodes_list, ambulance) # ambulance : 엠뷸런스 좌표
